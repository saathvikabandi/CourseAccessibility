...
This application is fully optimized for deployment on Vercel as a Single Page Application.
1. Import the repository into your Vercel account.
2. Add the `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` to the Vercel Environment Variables section.
3. Deploy! (Routing is natively handled via the included `vercel.json` rewrite file).
